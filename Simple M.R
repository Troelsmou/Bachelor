...
mySNP_nonmissing <- read_csv("./Data/ProFABA_GD.csv")

mySNP_nonmissing[,-1] %>%
  t() -> mySNP_nonmissing

#variances <- matrixStats::rowVars(as.matrix(mySNP_nonmissing))
#rmv <- which(variances == 0)
#mySNP_nonmissing <- mySNP_nonmissing[,-rmv]

monomorph <- apply(mySNP_nonmissing, 1, function(x) length(table(x)))
mySNP_nonmissing <- mySNP_nonmissing[!(monomorph == 1), ]

numLoci <- nrow(mySNP_nonmissing)
...
cat("Significance level is: ", 0.05/sum(simpleMeff), "\n")
